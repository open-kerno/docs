...
Use **`chrono`** for all date and time handling.
...
- `DateTime<Utc>` enforces UTC at the type level — no accidental local-time values.
- Built-in `serde` support serializes to ISO 8601 strings automatically.
- `chrono-tz` for named timezone conversions at display time.

**Do not use:**
- `std::time::SystemTime` for application timestamps — no timezone context, not human-readable.
- `NaiveDateTime` for storage or API fields — it has no timezone information by design.
- Manual string formatting with `format!()` — use `chrono`'s `to_rfc3339()` instead.
